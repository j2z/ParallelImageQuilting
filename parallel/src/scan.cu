...
__global__ void upward_sweep_kernel(int twod, int* array)
{
    int twod1 = twod * 2;
...
__global__ void downward_sweep_kernel(int twod, int* array)
{
    int twod1 = twod * 2;
    int index = (blockIdx.x * blockDim.x + threadIdx.x) * twod1;
...
void exclusive_scan(int* device_start, int length, int* device_result)
{
    ...
    for (int twod = 1; twod < roundedLength; twod *= 2)
    {
        ...
        int numThreads = roundedLength / twod / 2;
        ...
        upward_sweep_kernel<<<numBlocks, threadsPerBlock>>>(twod, device_result);
        cudaThreadSynchronize();
    }
    
    int zero = 0;

    cudaMemcpy(device_result + roundedLength - 1, &zero, sizeof(int), cudaMemcpyHostToDevice);
    ...
    for (int twod = roundedLength / 2; twod >= 1; twod /= 2)
    {
        ...
        downward_sweep_kernel<<<numBlocks, threadsPerBlock>>>(twod, device_result);
        cudaThreadSynchronize();
    }
...
__global__ void repeats_kernel(int N, int *array, int *output)
{
...
__global__ void put_repeat_indices(int N, int *prefixSum, int *repeat_indices)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < N - 1)
    {
        int val = prefixSum[index];
        if (val != prefixSum[index + 1])
        {
            repeat_indices[val] = index;
        }
    }
...
int find_repeats(int *device_input, int length, int *device_output) {
    ...
    int *repeats;
    cudaMalloc((void **)&repeats, nextPow2(length) * sizeof(int));

    int threadsPerBlock = 512;
    int numBlocks = (length + threadsPerBlock - 1) / threadsPerBlock;

    repeats_kernel<<<numBlocks, threadsPerBlock>>>(length, device_input, repeats);
    cudaThreadSynchronize();

    exclusive_scan(repeats, length, repeats);             // this is IN PLACE

    put_repeat_indices<<<numBlocks, threadsPerBlock>>>(length, repeats, device_output);
    cudaThreadSynchronize();
    
    int total;
    int a = cudaMemcpy(&total, repeats + length - 1, sizeof(int), cudaMemcpyDeviceToHost);

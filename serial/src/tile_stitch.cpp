...
void horizontal_stitch(CImg<int> im1, CImg<int> im2, int x1, int y1, int x2, int y2, int width, int height, int seam[])
{
  //basically transpose vertical stitching cuz I'm lazy
  int h = width;
  int w = height;
  float error_map[h][w];
  float min_seam[h][w];
  ...
  for (int channel = 0; channel < 3; channel++)
  {
    for (int j = 0; j < height; j++)
    {
      ...
      {
        float diff = im1(i,j+y1,channel) - im2(i,j+y1,channel);
        error_map[i][j] += diff * diff;
      }
    }
  }
  ...
  for (int j = 1; j < h; j++)
  {
    ...
    for (int i = 1; i < w-1; i++)
    {
      ...
      {
        if (min_seam[j-1][i-1] < min_seam[j-1][i+1])
        {
          // left is smallest
